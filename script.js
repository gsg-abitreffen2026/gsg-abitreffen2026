...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (button && navigator.share) {
      button.addEventListener("click", () => {
        navigator.share({
          ...
          console.error("Teilen abgebrochen oder nicht mÃ¶glich:", err);
        });
      });
    } else if (button) {
  ...
  // === Galerie & Newsletter Sichtbarkeit nach Status ===
  ...
  newsletterForm?.addEventListener("submit", function (e) {
    ...
    .then(data => {
      if (data.result === "success") {
        ...
        alert("Fehler bei der Anmeldung. Bitte versuche es spÃ¤ter.");
  ...
  kontaktForm?.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      alert("Fehler beim Absenden. Bitte versuche es spÃ¤ter.");
      console.error(error);
    }
  });

  // === Galerie-Login & Slideshow ===
